...
public class SysUserServiceImpl implements SysUserService {
    ...
    public SysUser detail(Long id) {
        ...
        List<SysRole> roleList = sysRoleMapper.selectRoleListByUserId(id);
    ...
    @Override
    public int addRole(Long userId, List<Long> roleIdList) {
        int addCount = 0;
        if (roleIdList != null && roleIdList.size() > 0) {
            for (Long roleId : roleIdList) {
                int existCount = sysUserMapper.selectCountByUserIdAndRoleId(userId, roleId);
                if (existCount <= 0) {
                    int count = sysUserMapper.insertUserRole(userId, roleId);
                    addCount += count;
                }
            }
        }
    ...
    @Override
    public int removeRole(Long userId, List<Long> roleIdList) {
        int removeCount = 0;
        if (roleIdList != null && roleIdList.size() > 0) {
            for (Long roleId : roleIdList) {
                removeCount = +sysUserMapper.deleteUserRole(userId, roleId);
            }
        }
        return removeCount;
    }
